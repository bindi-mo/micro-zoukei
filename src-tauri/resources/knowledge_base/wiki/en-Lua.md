...
### Passing JavaScript Objects as parameters

The Fengari Lua Virtual Machine does not convert Lua tables into JavaScript Objects.
Analogous to the above section, there may be times (such as when using alternative
graphics libs as also mentioned above) when the Javascript alternative library
requires a JavaScript Object as a parameter and substituting a Lua table won't work.
For example, the Babylon.js library debug scene inspector allows the programmer to
pass an JavaScript Object to customize the debug scene inspector display to display
the inspector as an overlay on the screen:

Passing a Lua table like this will not work: 
```
scene.debugLayer.show({overlay = true})
```

Instead, create a new JavaScript Object and set each of the key:value pairs in
the Javascript Object as follows:

```
debugOption = new(js.global.Object)
debugOption["overlay"] = true
scene.debugLayer:show(debugOption)
```
